...
if __name__=="__main__":
    ...
    if farm == "Grid":
        nRows = 5
        nTurbs = nRows**2
        spacing = 10   # turbine grid spacing in diameters
    ...
    for i in range(len(space)):
        ...
        prob.driver.add_desvar('d_paramH1', lower=np.array([1.0, 1.0, d_param[nPoints-1]]), upper=None, scaler=1.0)
        prob.driver.add_desvar('t_paramH1', lower=np.ones(nPoints)*.001, upper=None, scaler=1.0)
        prob.driver.add_desvar('d_paramH2', lower=np.array([1.0, 1.0, d_param[nPoints-1]-0.0001]), upper=None, scaler=1.0)
        ...
        np.savetxt('XYZ_grid_%s_ws_exp_0.12_RC.txt'%space[i], np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
        np.savetxt('dt_grid_%s_ws_exp_0.12_RC.txt'%space[i], np.c_[prob['d_paramH1'], prob['d_paramH2'], prob['t_paramH1'], prob['t_paramH2']], header="d_paramH1, d_paramH2, t_paramH1, t_paramH2")
