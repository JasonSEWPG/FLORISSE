...
if __name__=="__main__":
    ...
    shear_ex = np.array([0.22,0.23,0.24,0.25,0.26,0.27,0.28,0.29,0.3])
    ...
    for i in range(len(shear_ex)):
        ...
        # prob.driver.opt_settings['Summary file'] = 'SNOPT_3.1_5_%s_SNOPT_2_6.3.out'%shearExp
        ...
        # # --- Design Variables ---
        prob.driver.add_desvar('turbineH1', lower=rotor_diameter/2.+10, upper=160., scaler=1.0)
        prob.driver.add_desvar('turbineH2', lower=rotor_diameter/2.+10, upper=160., scaler=1.0)
        ...
        prob.driver.add_desvar('d_paramH1', lower=np.array([1.0, 1.0, d_param[nPoints-1]]), upper=np.ones(nPoints)*6.3, scaler=1.0)
        prob.driver.add_desvar('t_paramH1', lower=np.ones(nPoints)*.001, upper=np.ones(nPoints)*0.05, scaler=1.0)
        prob.driver.add_desvar('d_paramH2', lower=np.array([1.0, 1.0, d_param[nPoints-1]]), upper=np.ones(nPoints)*6.3, scaler=1.0)
        ...
        np.savetxt('SAME_XYZ_3.1_5_%s_SP_2_6.3.txt'%shearExp, np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
        # np.savetxt('dt_3.1_5_%s_SNOPT_2_6.3.txt'%shearExp, np.c_[prob['d_paramH1'], prob['d_paramH2'], prob['t_paramH1'], prob['t_paramH2']], header="d_paramH1, d_paramH2, t_paramH1, t_paramH2")
        np.savetxt('SAME_dt_3.1_5_%s_SP_2_6.3.txt'%shearExp, np.c_[prob['d_paramH1'], prob['t_paramH1']], header="d_paramH1, t_paramH1")
