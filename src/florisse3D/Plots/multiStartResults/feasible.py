...
if __name__ == '__main__':
    ...
    # XYZfile = 'XYZ_WARM_SHEAR_2_2GROUP_YAW_0.09.txt'
    # optXYZ = open(XYZfile)
    ...
    turbineZ = np.ones(nTurbs)*90.

    shearExponent = np.array([0.08,0.09,0.1,0.11,0.12,0.13,0.14,0.15,0.16,0.17 \
                0.18,0.19,0.2,0.21,0.22,0.23,0.24,0.25,0.26,0.27,0.28,0.29,0.3])

    SpacingCon = np.zeros(len(shearExponent))
    BoundaryCon = np.zeros(len(shearExponent))
    frequencyCon = np.zeros(len(shearExponent))
    bucklingCon_speed = np.zeros(len(shearExponent))
    bucklingCon_thrust = np.zeros(len(shearExponent))

    for j in range(len(shearExponent)):

        shearExp = shearExponent[j]

        thicknessfile = 'thickness_WARM_SHEAR_GRID_%s.txt'%shearExp
        optthickness = open(thicknessfile)
        optimizedthickness = np.loadtxt(optthickness)
        c1 = optimizedthickness[:,0]
        c2 = optimizedthickness[:,1]
        c3 = optimizedthickness[:,2]
        ...
        diameterfile = 'diameter_WARM_SHEAR_GRID_%s.txt'%shearExp
        optdiameter = open(diameterfile)
        optimizeddiameter = np.loadtxt(optdiameter)
        d1 = optimizeddiameter[:,0]
        d2 = optimizeddiameter[:,1]
        d3 = optimizeddiameter[:,2]
        ...
        # yawfile = 'yaw_WARM_SHEAR_2_2GROUP_YAW_0.09.txt'
        # optyaw = open(yawfile)
        # optimizedyaw = np.loadtxt(optyaw)
        # yaws = np.zeros((nDirections, nTurbs))
        # for i in range(nDirections):
        #     yaws[i] = optimizedyaw[i,:]
        ...
        for j in range(nGroups):
            print 'Group %s'%(j+1)
            print 'Max Speed: Shell Buckling: ', prob['Tower%s_max_speed.shell_buckling'%j]
            print 'Max Thrust: Shell Buckling: ', prob['Tower%s_max_thrust.shell_buckling'%j]
            print 'Frequency: ', prob['Tower%s_max_thrust.freq'%j]
