...
if __name__=="__main__":
    ...
    gridfile = 'src/florisse3D/Plots/multiStartResults/density_grid.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid = optimizedgrid[:,0]


    layoutfile = 'src/florisse3D/Plots/multiStartResults/density_layout.txt'
    optlayout = open(layoutfile)
    optimizedlayout = np.loadtxt(optlayout)
    layout = optimizedlayout[:]

    group1file = 'src/florisse3D/Plots/multiStartResults/density_1group.txt'
    opt1group = open(group1file)
    optimized1group= np.loadtxt(opt1group)
    group1 = optimized1group#[:,0]


    group2file = 'src/florisse3D/Plots/multiStartResults/density_2group.txt'
    opt2group = open(group2file)
    optimized2group= np.loadtxt(opt2group)
    group2 = optimized2group#[:,0]
    #
    #
    group1_yaw_file = 'src/florisse3D/Plots/multiStartResults/density_1groupyaw.txt'
    opt1group_yaw = open(group1_yaw_file)
    optimized1group_yaw= np.loadtxt(opt1group_yaw)
    group1_yaw = optimized1group_yaw#[:,0]

    group2_yaw_file = 'src/florisse3D/Plots/multiStartResults/density_2groupyaw.txt'
