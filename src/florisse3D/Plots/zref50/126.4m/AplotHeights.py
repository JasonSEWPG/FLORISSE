...
if __name__=="__main__":
    file = 'src/florisse3D/Plots/zref50/126.4m/A1heights.txt'
    opt = open(file)
    optimized = np.loadtxt(opt)
    group10 = optimized[:]

    file = 'src/florisse3D/Plots/zref50/126.4m/A2heights.txt'
    opt = open(file)
    optimized = np.loadtxt(opt)
    ...
    ax.plot(shearExp[2:6], group20_2[2:6], 'r',linewidth=3,label='hub height, group 2')
    ax.plot(shearExp[0:3], group20_1[0:3], 'r',linewidth=1.5)
    ax.plot(shearExp[5:23], group20_1[5:23], 'r',linewidth=1.5)
    ax.plot([0,1],[90.,90.],'--k')#, label='90 m')
    ...
    plt.xlabel('Shear Exponent')
    plt.ylabel('Optimized Hub Height (m)')
    plt.title('Big Farm: Large Rotor: Heights')
    plt.legend(loc=4)
    plt.tight_layout()
    plt.axis([0.075,0.305,0.,120.])
