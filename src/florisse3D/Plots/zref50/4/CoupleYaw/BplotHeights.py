...
if __name__=="__main__":
    file = 'src/florisse3D/Plots/zref50/4/CoupleYaw/B1heights.txt'
    opt = open(file)
    optimized = np.loadtxt(opt)
    group10 = optimized[:]

    file = 'src/florisse3D/Plots/zref50/4/CoupleYaw/B2heights.txt'
    opt = open(file)
    optimized = np.loadtxt(opt)
    ...
    plt.xlabel('Shear Exponent')
    plt.ylabel('Optimized Hub Height (m)')
    plt.title('126.4 m Rotor Diameter, 2 Groups')
    plt.legend(loc=4)
    plt.tight_layout()
    plt.axis([0.075,0.305,0.,120.])
    #
