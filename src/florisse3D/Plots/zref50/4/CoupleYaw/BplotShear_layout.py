...
if __name__=="__main__":
    layoutfile = 'src/florisse3D/Plots/zref50/4/CoupleYaw/Bshear_layout.txt'
    optlayout = open(layoutfile)
    optimizedlayout = np.loadtxt(optlayout)
    layout = optimizedlayout[:,1]

    #
    layoutfile = 'src/florisse3D/Plots/zref50/4/CoupleYaw/Bshear_layout_1.txt'
    optlayout = open(layoutfile)
    optimizedlayout = np.loadtxt(optlayout)
    layout_1group = optimizedlayout[:,1]
    #

    layoutfile = 'src/florisse3D/Plots/zref50/4/CoupleYaw/Bshear_layout_2.txt'
    optlayout = open(layoutfile)
    optimizedlayout = np.loadtxt(optlayout)
    ...
    plt.ylabel('COE ($/MWh)')
    plt.title('Farm 2: Layout and Yaw')
    plt.tight_layout()
    plt.show()
