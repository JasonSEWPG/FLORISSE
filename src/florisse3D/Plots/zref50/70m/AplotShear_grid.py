...
if __name__=="__main__":
    gridfile = 'src/florisse3D/Plots/zref50/70m/Ashear_grid.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid = optimizedgrid[:,1]

    gridfile_yaw = 'src/florisse3D/Plots/zref50/70m/Ashear_grid_YAW.txt'
    optgrid_yaw = open(gridfile_yaw)
    optimizedgrid_yaw = np.loadtxt(optgrid_yaw)
    grid_yaw = optimizedgrid_yaw[:,1]
    #
    gridfile = 'src/florisse3D/Plots/zref50/70m/Ashear_grid_1.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid_1group = optimizedgrid[:,1]
    #
    gridfile_yaw = 'src/florisse3D/Plots/zref50/70m/Ashear_grid_1_YAW.txt'
    optgrid_yaw = open(gridfile_yaw)
    optimizedgrid_yaw = np.loadtxt(optgrid_yaw)
    grid_1group_yaw = optimizedgrid_yaw[:,1]
    #
    gridfile = 'src/florisse3D/Plots/zref50/70m/Ashear_grid_2.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid_2group = optimizedgrid[:,1]
    #
    gridfile_yaw = 'src/florisse3D/Plots/zref50/70m/Ashear_grid_2_YAW.txt'
    optgrid_yaw = open(gridfile_yaw)
    optimizedgrid_yaw = np.loadtxt(optgrid_yaw)
    ...
    plt.ylabel('COE ($/MWh)')
    plt.title('Big Farm: Small Rotor')
    plt.tight_layout()
    plt.show()
