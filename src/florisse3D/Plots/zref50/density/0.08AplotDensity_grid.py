...
if __name__=="__main__":
    gridfile = 'src/florisse3D/Plots/zref50/density/0.08A.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid = optimizedgrid[:,1]

    gridfile = 'src/florisse3D/Plots/zref50/density/0.08A_1.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid_1group = optimizedgrid[:,1]
    #
    gridfile = 'src/florisse3D/Plots/zref50/density/0.08A_2.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    ...
    plt.title('0.08 Shear Exponent: Small Rotor', y=1.15)
    plt.tight_layout()
    plt.show()
