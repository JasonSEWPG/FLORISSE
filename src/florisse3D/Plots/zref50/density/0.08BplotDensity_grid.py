...
if __name__=="__main__":
    gridfile = 'src/florisse3D/Plots/zref50/density/0.08B.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid = optimizedgrid[:,1]

    gridfile = 'src/florisse3D/Plots/zref50/density/0.08B_1.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid_1group = optimizedgrid[:,1]
    #
    gridfile = 'src/florisse3D/Plots/zref50/density/0.08B_2.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    ...
    ax2 = ax1.twiny()
    ax1.plot(density, grid, 'ob', label='baseline')
    ax1.plot(density, grid_1group, 'or', label='1 group')
    ax1.plot(density, grid_2group, 'ok', label='2 groups')
    ax1.plot(density[0:7], grid_2group[0:7], 'ok', markeredgecolor='red')
    # ax1.plot(density[9], grid[9], 'ob', label='grid')
    # ax1.plot(density[0], grid_1group[0], 'or', label='1 group, grid')
    # ax1.plot(density[2:10], grid_2group[2:10], 'ok', label='2 groups, grid')
    ...
    plt.title('0.08 Shear Exponent: Large Rotor', y=1.15)
    plt.tight_layout()
    plt.show()
