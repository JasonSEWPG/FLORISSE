...
if __name__=="__main__":
    gridfile = 'src/florisse3D/Plots/zref50/density/0.25B.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid = optimizedgrid[:,1]

    gridfile = 'src/florisse3D/Plots/zref50/density/0.25B_1.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid_1group = optimizedgrid[:,1]
    #
    gridfile = 'src/florisse3D/Plots/zref50/density/0.25B_2.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    ...
    ax1.plot(density[0:5], grid_2group[0:5], 'ok', markeredgecolor='red')
    ...
    plt.title('0.25 Shear Exponent: Large Rotor', y=1.15)
    plt.tight_layout()
    plt.show()
