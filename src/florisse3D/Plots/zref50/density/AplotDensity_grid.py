...
if __name__=="__main__":
    gridfile = 'src/florisse3D/Plots/zref50/density/A.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid = optimizedgrid[:,1]

    gridfile = 'src/florisse3D/Plots/zref50/density/A_1.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    grid_1group = optimizedgrid[:,1]
    #
    gridfile = 'src/florisse3D/Plots/zref50/density/A_2.txt'
    optgrid = open(gridfile)
    optimizedgrid = np.loadtxt(optgrid)
    ...
    ax2 = ax1.twiny()
    ax1.plot(density[0:10], grid[0:10], 'ob', label='baseline')
    ax1.plot(density[0:10], grid_1group[0:10], 'or', label='1 group')
    ax1.plot(density[0:10], grid_2group[0:10], 'ok', label='2 groups')
    # ax1.plot(density[0:10], grid[0:10], 'ob', label='baseline')
    # ax1.plot(density[0:10], grid_1group[0:10], '*r', label='1 optimized height')
    # ax1.plot(density[0:10], grid_2group[0:10], '^k', label='2 height groups')
    ...
    plt.title('0.15 Shear Exponent: Small Rotor', y=1.15)
    # plt.title('Varied Turbine Density', y=1.15)
    plt.tight_layout()
    plt.show()
