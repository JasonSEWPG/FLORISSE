...
if __name__ == '__main__':
    ...
    for i in range(nGroups):
        ...
        # prob.driver.add_constraint('rotorDiameter%s'%i, upper=150.)
    ...
    for i in range(nGroups):
        prob['rotor_nacelle_costs%s.blade_mass'%i] = 10000.
        prob['rotor_nacelle_costs%s.rotor_torque'%i] = 10000.
        prob['Tower%s_max_thrust.Vel'%i] = 12.
        prob['Tower%s_max_speed.Vel'%i] = 70.
        prob['Tower%s_max_thrust.Fx'%i] = 600000.
        prob['Tower%s_max_speed.Fx'%i] = 60000.
    # root.connect('Rotor%s.I_all_blades'%i, 'split_I%s.Array'%i)
    # root.connect('split_I%s.output%s'%(i,2),'Tower%s_max_speed.It'%i)
    ...
    print 'AEPstart: ', AEPstart
    print 'COEstart: ', COEstart
