...
if __name__ == '__main__':
    ...
    d_param = np.array([ 5.05864404,  3.87      ,  3.87      ])
    t_param = np.array([ 0.00728175,  0.00566108,  0.00502558])
    ...
    rotor2 = 48.
    rotorDiameter = np.array([180., rotor2])
    turbineZ = np.array([116.635254045,88.9226661283])
    rotor2 = np.linspace(23.,180.,300)

    """OpenMDAO"""
    COEgradient = np.zeros(300)
    for k in range(300):
        rotorDiameter = np.array([180., rotor2[k]])
        prob = Problem()
        ...
        step = 0.01
        prob['rotorDiameter1'] = rotor2[k] + step
        ...
        costGrad = (costHigh-costLow)/step
        AEPgrad = (AEPHigh-AEPLow)/step
        COEgrad = (COEHigh-COELow)/step

        print 'costGrad: ', costGrad
        print 'AEPGrad: ', AEPgrad
        print 'COEgrad: ', COEgrad

        COEgradient[k] = COEgrad

    plt.plot(rotor2,COEgradient)
    plt.show()
