...
if __name__ == '__main__':
    ...
    # try:
    #     os.makedirs('layout_rotor')
    # except OSError:
    #     if not os.path.isdir('layout_rotor'):
    #         raise
    #
    # try:
    #     os.makedirs('layout_rotor/%s'%shearExp)
    # except OSError:
    #     if not os.path.isdir('layout_rotor/%s'%shearExp):
    #         raise
    #
    # try:
    #     os.makedirs('layout_rotor/%s/%s'%(shearExp,spacing_multiplier))
    # except OSError:
    #     if not os.path.isdir('layout_rotor/%s/%s'%(shearExp,spacing_multiplier)):
    #         raise
    ...
    # prob.driver.opt_settings['Summary file'] = 'layout_rotor/%s/%s/summary_%s_%s.out'%(shearExp,spacing_multiplier,jobID,taskID)
    # prob.driver.opt_settings['Print file'] = 'layout_rotor/%s/%s/verify_%s_%s.out'%(shearExp,spacing_multiplier,jobID,taskID)
    prob.driver.opt_settings['Summary file'] = 'sum.out'
    prob.driver.opt_settings['Print file'] = 'print.out'
    ...
    # np.savetxt('layout_rotor/%s/%s/XYZ_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
    # np.savetxt('layout_rotor/%s/%s/XYstart_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[turbineXstart, turbineYstart], header="turbineXstart, turbineYstart")
    # np.savetxt('layout_rotor/%s/%s/diameter_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[diameters], header="diameters")
    # np.savetxt('layout_rotor/%s/%s/thickness_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[thicknesses], header="thicknesses")
    # np.savetxt('layout_rotor/%s/%s/COE_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[prob['COE'], prob['AEP']])
    # np.savetxt('layout_rotor/%s/%s/power_diam_%s_%s.txt'%(shearExp,spacing_multiplier,jobID, taskID), np.c_[ratedPowers,rotorDiameters])
