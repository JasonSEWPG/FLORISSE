...
if __name__ == '__main__':
    ...
    # turbineX_bounds = np.array([0.,300.,600.,900.])
    # turbineY_bounds = np.array([0.,10.,10.,0.])
    ...
    if windData == "Amalia":
        ...
        windDirections = np.array([0.,30.,60.,90.,120.,150.])
    ...
    turbineX = turbineX_bounds+np.random.rand(nTurbs)*(spacing*rotor_diameter-rotor_diameter/2.)-(spacing*rotor_diameter-rotor_diameter/2.)/2.
    turbineY = turbineY_bounds+np.random.rand(nTurbs)*(spacing*rotor_diameter-rotor_diameter/2.)-(spacing*rotor_diameter-rotor_diameter/2.)/2.
    # turbineX = np.random.rand(nTurbs)*(xmax-xmin)+xmin
    # turbineY = np.random.rand(nTurbs)*(ymax-ymin)+ymin
    ...
    turbineXorig = np.zeros(nTurbs)
    turbineYorig = np.zeros(nTurbs)
    for i in range(nTurbs):
        turbineXorig[i] = turbineX[i]
        turbineYorig[i] = turbineY[i]
    ...
    for i in range(nGroups):
        print 'Group %s:'%i
        print 'Max Thrust'
        print prob['Tower%s_max_thrust.shell_buckling'%i]
        print 'Max Speed'
        print prob['Tower%s_max_speed.shell_buckling'%i]

    print 'Frequency: '
    for i in range(nGroups):
        print 'Group %s:'%i
        print prob['Tower%s_max_thrust.freq'%i]
    ...
    np.savetxt('check_XYZ1.txt', np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
    np.savetxt('check_d1.txt', np.c_[diameters], header="diameters")
    np.savetxt('check_t1.txt', np.c_[thicknesses], header="thicknesses")
    np.savetxt('check_yaw1.txt', np.c_[yawAngles], header="yaw")
    ...
    color = (0,0.6,0.8)
    for j in range(nTurbs):
        ax.add_artist(Circle(xy=(turbineXorig[j],turbineYorig[j]),
                  radius=rotor_diameter/2., fill=False, edgecolor=color))

    ax.axis([xmin-200,xmax+200,ymin-200,ymax+200])
    plt.title('original')
    plt.axis('off')
    plt.show()
