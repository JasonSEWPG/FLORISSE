...
if __name__ == '__main__':
    ...
    d_paramH2 = np.array([5.0, 4.0, 3.0])
    t_paramH2 = np.array([0.03,0.02,0.015])
    ...
    root.add('AEPGroup', AEPGroup(nTurbs, nDirections=1,
                use_rotor_components=True, datasize=0, differentiable=True,
                nSamples=0), promotes=['*'])
    ...
    prob.driver.opt_settings['Summary file'] = 'AAoneDir.out'
    ...
    np.savetxt('AAoneDirXYZ2.txt', np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
    np.savetxt('AAoneDirdt2.txt', np.c_[prob['d_paramH1'], prob['d_paramH2'], prob['t_paramH1'], prob['t_paramH2'],])
