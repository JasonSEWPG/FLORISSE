...
if __name__ == '__main__':
    ...
    shearExp = float(argv[2])
    nGroups = int(argv[3])
    withYaw = int(argv[4])
    jobID = int(argv[5])
    taskID = int(argv[6])
    layout = (argv[7]) # grid, just_layout, height_groups
    optimization = (argv[8]) #spacing, shearExp
    ...
    nRows = 5
    nTurbs = nRows**2
    spacing = spacing  # turbine grid spacing in diameters
    ...
    #Create the folders (if necessary) to save the results
    try:
        os.makedirs('%s'%optimization)
    except OSError:
        if not os.path.isdir('%s'%optimization):
            raise

    try:
        os.makedirs('%s/%s'%(optimization, layout))
    except OSError:
        if not os.path.isdir('%s/%s'%(optimization, layout)):
            raise

    try:
        os.makedirs('%s/%s/%s'%(optimization, layout, nGroups))
    except OSError:
        if not os.path.isdir('%s/%s/%s'%(optimization, layout, nGroups)):
            raise

    if optimization == 'spacing':
        try:
            os.makedirs('%s/%s/%s/%s'%(optimization, layout, nGroups, spacing))
        except OSError:
            if not os.path.isdir('%s/%s/%s/%s'%(optimization, layout, nGroups, spacing)):
                raise
    elif optimization == 'shearExp':
        try:
            os.makedirs('%s/%s/%s/%s'%(optimization, layout, nGroups, shearExp))
        except OSError:
            if not os.path.isdir('%s/%s/%s/%s'%(optimization, layout, nGroups, shearExp)):
                raise
    ...
    for i in range(nGroups):
        ...
        if layout == 'grid':
            root.add('turbineH%s'%i, IndepVarComp('turbineH%s'%i, 90.), promotes=['*'])
        elif layout == 'just_layout':
            root.add('turbineH%s'%i, IndepVarComp('turbineH%s'%i, 90.), promotes=['*'])
        elif layout == 'height_groups':
    ...
    if optimization == 'spacing':
        prob.driver.opt_settings['Summary file'] = '%s/%s/%s/%s/summary_%s_%s_%s.out'%(optimization, layout, nGroups, spacing, jobID, taskID, withYaw)
        prob.driver.opt_settings['Print file'] = '%s/%s/%s/%s/print_%s_%s_%s.out'%(optimization, layout, nGroups, spacing, jobID, taskID, withYaw)
    elif optimization == 'shearExp':
        prob.driver.opt_settings['Summary file'] = '%s/%s/%s/%s/summary_%s_%s_%s'%(optimization, layout, nGroups, shearExp, jobID, taskID, withYaw)
        prob.driver.opt_settings['Print file'] = '%s/%s/%s/%s/print_%s_%s_%s'%(optimization, layout, nGroups, shearExp, jobID, taskID, withYaw)

    prob.driver.opt_settings['Major iterations limit'] = 2000
    ...
    if optimization == 'spacing':
        np.savetxt('%s/%s/%s/%s/XYZ_%s_%s_%s.txt'%(optimization, layout, nGroups, spacing, jobID, taskID, withYaw), np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
        np.savetxt('%s/%s/%s/%s/diameter_%s_%s_%s.txt'%(optimization, layout, nGroups, spacing, jobID, taskID, withYaw), np.c_[diameters], header="diameters")
        np.savetxt('%s/%s/%s/%s/thickness_%s_%s_%s.txt'%(optimization, layout, nGroups, spacing, jobID, taskID, withYaw), np.c_[thicknesses], header="thicknesses")
        np.savetxt('%s/%s/%s/%s/yaw_%s_%s_%s.txt'%(optimization, layout, nGroups, spacing, jobID, taskID, withYaw), np.c_[yawAngles], header="yaw")
        np.savetxt('%s/%s/%s/%s/COE_%s_%s_%s.txt'%(optimization, layout, nGroups, spacing, jobID, taskID, withYaw), np.c_[prob['COE']])
    elif optimization == 'shearExp':
        np.savetxt('%s/%s/%s/%s/XYZ_%s_%s_%s.txt'%(optimization, layout, nGroups, shearExp, jobID, taskID, withYaw), np.c_[prob['turbineX'], prob['turbineY'], prob['turbineZ']], header="turbineX, turbineY, turbineZ")
        np.savetxt('%s/%s/%s/%s/diameter_%s_%s_%s.txt'%(optimization, layout, nGroups, shearExp, jobID, taskID, withYaw), np.c_[diameters], header="diameters")
        np.savetxt('%s/%s/%s/%s/thickness_%s_%s_%s.txt'%(optimization, layout, nGroups, shearExp, jobID, taskID, withYaw), np.c_[thicknesses], header="thicknesses")
        np.savetxt('%s/%s/%s/%s/yaw_%s_%s_%s.txt'%(optimization, layout, nGroups, shearExp, jobID, taskID, withYaw), np.c_[yawAngles], header="yaw")
        np.savetxt('%s/%s/%s/%s/COE_%s_%s_%s.txt'%(optimization, layout, nGroups, shearExp, jobID, taskID, withYaw), np.c_[prob['COE']])
