...
if __name__=="__main__":
    ...
    prob.driver.opt_settings['Summary file'] = 'SNOPT_TEST.out'
    ...
    # # --- Design Variables ---
    prob.driver.add_desvar('turbineH1', lower=rotor_diameter/2.+10, upper=130., scaler=1.0E-1)
    prob.driver.add_desvar('turbineH2', lower=rotor_diameter/2.+10, upper=130., scaler=1.0E-1)
    ...
    prob.driver.add_desvar('d_paramH1', lower=np.array([1.0, 1.0, d_param[2]]), upper=np.ones(3)*6.3, scaler=1.0E-1)
    prob.driver.add_desvar('t_paramH1', lower=np.ones(3)*.001, upper=np.ones(3)*0.05, scaler=1.0E1)
    prob.driver.add_desvar('d_paramH2', lower=np.array([1.0, 1.0, d_param[2]]), upper=np.ones(3)*6.3, scaler=1.0E-1)
    prob.driver.add_desvar('t_paramH2', lower=np.ones(3)*.001, upper=np.ones(3)*0.05, scaler=1.0E1)
    ...
    print 'Tower Mass H1: ', prob['MassH1.mass']
    print 'Tower Mass H2: ', prob['MassH2.mass']
